...
import { generateHeadCoachCode } from "@/app/admin/actions";
import type { RoleInviteCode } from "@/lib/types";

export function RoleCodesPanel({ initialCodes }: { initialCodes: RoleInviteCode[] }) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  // initialCodes viene del server component; router.refresh() tras generar
  ...
  function handleGenerate() {
    ...
    startTransition(async () => {
      const result = await generateHeadCoachCode();
      if (result.error) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        <Plus size={18} />
        {isPending ? "Generando…" : "Generar código de Head Coach"}
      </Button>
      ...
      {activeCodes.length > 0 ? (
        ...
        <Card>
          <p className="text-sm text-text-secondary">
            Genera un código y compártelo con la persona que va a registrarse
            como Head Coach. Lo usará en el paso de registro.
          </p>
        </Card>
      )}
    </div>
